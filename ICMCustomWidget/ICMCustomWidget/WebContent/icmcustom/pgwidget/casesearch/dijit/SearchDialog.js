...
define([ 
	...
	return declare("icmcustom.pgwidget.casesearch.dijit.SearchDialogPane", [BaseDialog], {
		...
		constructor: function(/*Object*/args){
			...
			this._businessObjCriteriaWidgets = [];
			...
			this.allBOProps = this._getBOSearchProps(args.searchViewPropDefs);
		...
		postCreate: function(){
			...
			this.searchButton = this.addButton("Export", "onSearch", true, false, "SEARCH");
		...
		_retrieveBaseAttributeDefinitions: function() {			
			...
			this._addBusinessObjectSelectionField();
		...
		onSearch: function(){
			...
			this._searchDefinition.hasBOAdvanceSearch = false;
			
			var criterion = []; 
			var bo_props_criterion = [];
			...
			dojo.forEach(this._businessObjCriteriaWidgets, function(w, i){
				if(this._isWidgetValueSet(w) || w._operator == "NULL" || w._operator == "NOTNULL"){
					var criteria = this.createCriterion(w, i, "more");
					bo_props_criterion = bo_props_criterion.concat(criteria);
				}
			}, this);			
			
			if(this.hasBOSearchCriteria() && bo_props_criterion.length > 0)
			{
				this._searchDefinition.hasBOAdvanceSearch = true;
				var  bo_criterion = new Array();
				bo_criterion = this.createBOCriterion(this._businessObjectAttr, bo_props_criterion);
				
				if (bo_criterion && bo_criterion.length > 0)
				{
					criterion = criterion.concat(bo_criterion);	
				}
			}
		...
		hasBOSearchCriteria: function()
		{
			if (this._businessObjectAttr && 
					this._businessObjCriteriaWidgets && this._businessObjCriteriaWidgets.length > 0)
				return true;
			else
				return false;
		},	
		...
		createCriterion: function(/*dijit._Widget*/widget, /*Integer*/index, /*String*/prefix) {
			...
			if (attDef.id == "CmAcmCaseIdentifier"){
				selectedOperator = this._searchUtil.OPERATOR.STARTS_WITH;
			}
		...
		createBOCriterion: function(/*AttributeDefinition*/attDef, bo_props) {
			this.logEntry("Case SearchContentPane createBOCriterion");
			...
			var values = [];//ignore values for BO type
			var selectedOperator = "SATISFIES";//this.getSearchPropertyOperator(symbolicName);		
			var criterion = this._searchUtil.createSearchCriterionModel(attDef, format, operators, selectedOperator, values);
			
			criterion.object_search_criteria = bo_props;

			criterions.push(criterion);
			this.logExit("Case SearchContentPane createBOCriterion", criterions);
			return criterions;
		},		
		...
		_convertDateValue: function(attDef, values,  selectedOperator, isInterval) {
			...
			//masking time portion of date
			if(dataType == "xs:timestamp" && values) {
				if(dojo.isArray(values)) {
					for(i=0; i< values.length; i++) {
						if(values[i]) {
							value = new Date(values[i]);
							if(!isNaN(value.getTime())) {	//checking for valid date
								values[i] = dojo.date.stamp.toISOString(new Date(value.toDateString()));
							}
						}
					}
				} else {
					...
					if(!isNaN(value.getTime())) {	//checking for valid date
						values = dojo.date.stamp.toISOString(new Date(value.toDateString()));
					}
				}
			}
		...
		onCaseTypeChange: function(/*String*/value){
			...
			this._destroyWidgets(this._businessObjCriteriaWidgets);
			...
			dojo.empty(this.businessObjCriteria);
			...
			if(this._caseType) {
				...
				var caseBOProps = this._getCaseBOProperties();
				this._populateBOSelection(caseBOProps);
			} else {
				this._addCrossCaseTypePropertyFields();
				this._populateBOSelection(this.allBOProps);
			}
			this.logExit("Case SearchDialogPane onCaseTypeChange");
		},
		
		onBusinessObjectChange: function(/*String*/value){
			this.logEntry("Case SearchDialogPane onBusinessObjectChange");
			
			if(this._businessObjectName == value){ return; }
			...
			this._businessObjectAttr = this.solution.attributeDefinitionsById[value];
			
			if (this._businessObjectAttr) {
				this._createDependentAttrDef(this._businessObjectAttr);				
			}

			this.logExit("Case SearchDialogPane onBusinessObjectChange");
		},		
		...
		_addBusinessObjectSelectionField: function(){
			...
			var id = (new Date()).getTime() + "boSelectNode";	
			dojo.attr(this.boSelectNodeLabel, "for", id);
			this._boTypeInputField = new FilteringSelect({
				id: id,
				name: "boSelectNode",
				required: true,
				store: typeStore,
				...
				onChange: dojo.hitch(this, this.onBusinessObjectChange)
			});

			this.connect(this._boTypeInputField, "validate", dojo.hitch(this, this._onValidate));
			this._boTypeInputField.placeAt(this.boSelectNode);

			this._addAllBOProperties();
			
			// show and populate the BO selection drop down only if there BO props exist 
			if (this.allBOProps && this.allBOProps.length > 0)
			{
				this._boTypeInputField.attr("value", this.allBOProps[0].name);
				dojo.attr(this._boTypeInputField, "value", this.allBOProps[0].name);
				this._boTypeInputField.startup();			
			}
			else
			{
				this.boPropertiesSection.style.visibility = 'hidden';
			}
		},
		
		_addAllBOProperties: function() {
			if (!this.allBOProps) { return; }
			
			this.allBOProps.sort(function(a, b){
				return a.name < b.name ? -1 : 1;
			});
			
			dojo.forEach(this.allBOProps, function(type){
				this._boTypeInputField.store.newItem({
					id: type.id,
					name: type.name
				});
			}, this);
		...
		_populateBOSelection: function(boProps)
		{
			if (!boProps) 
			{
				this.boPropertiesSection.style.visibility = 'hidden';
			...
			else if (boProps.length == 0)
			{
				this.boPropertiesSection.style.visibility = 'hidden';
				return;
			}
			
			this.boPropertiesSection.style.visibility = 'visible';
			
			boProps.sort(function(a, b){
				return a.name < b.name ? -1 : 1;
			});
			
			// delete existing store items, if any
			var items = this._boTypeInputField.store._arrayOfAllItems;
			for ( var i = 0; i < items.length; i++) 
			{
				if (items[i] != null)
				{
					var storeItem = items[i];
					this._boTypeInputField.store.deleteItem(storeItem);	
				}
			}
			
			this._boTypeInputField.store.save();
			
			// recreate new store items
			dojo.forEach(boProps, function(type){
				this._boTypeInputField.store.newItem({
					id: type.id,
					name: type.name
				});
		...
		_getCaseBOProperties: function() 
		{
			var i; var props = [];
			
			var searchViewProperties = this._getSearchViewPropertyNames(this._caseType);
			
			for (i = 0; i < searchViewProperties.length; i++)
			{
				var name = searchViewProperties[i]; 
				var attDef = this._searchUtil.getAttributeDefinitionByID(this.allBOProps, name);
				if (attDef && attDef.dataType == "xs:object") 
				{
					props.push(attDef);
				}
			}
		...
		_addBOCasePropertyFieldsDeferred: function(/*AttributeDefinition[]*/attributeDefinitions)
		{
		...
		_addCasePropertyFieldsByAttDefs: function(/*AttributeDefinition[]*/attributeDefinitions) {
			dojo.forEach(attributeDefinitions, function(attDef, index){
				if (attDef.dataType != "xs:object")	// don't include business object search properties here
		...
		*/
		_onAdd: function() {
			...
			if(this.isShowAllProperties && this._caseTypeName == "all") {
				...
				attDefs = this._getNonBOSearchProps(this.searchViewPropDefs);
			} else {
				attDefs = this._getNonBOSearchProps(this._attributeDefinitions[this._caseTypeName]);
		...
		_onBOAdd: function() {
			...
			this._setUpBOControlButtons(w);
			
			this.businessObjCriteria.appendChild(w.domNode);
			this._businessObjCriteriaWidgets.push(w);
		},
		...
		_setUpBOControlButtons: function(wid) {
			var self = this;
			//Need to deprecate dojo.connect and replace with newer API
			dojo.connect(wid.domNode, "onmouseover", function() {
				domClass.add(wid.domNode, "attributeDefintionWidgetHover");
		...
		_removeBOWidget: function(wid) {
			var i, isDisabled = false;
			for(i = 0; i < this._businessObjCriteriaWidgets.length; i++) {
				if(this._businessObjCriteriaWidgets[i] == wid) {
					this._businessObjCriteriaWidgets.splice(i, 1);
					break;
				}
			}
			this.businessObjCriteria.removeChild(wid.domNode);
			this._destroyWidgets(wid);
			this._onValidate();
		},
		
		_createDependentAttrDef: function(attDef){
			if(this.solution.dependentObjectClassesById && this.solution.dependentObjectClassesById[attDef.requiredClass]){
				var items = new Array();	
				dojo.forEach(this.solution.dependentObjectClassesById[attDef.requiredClass].attributeDefinitions, function(property) {
					if(property.dataType != "xs:guid")
						items.push(property);
				});
				this.boAttrDef = items;
			}
		},		
		
		// get all business object properties
		_getBOSearchProps: function(searchProps) {
			var boProps = [];
			for (i = 0; i < searchProps.length; i++) 
			{
				if (searchProps[i].dataType == "xs:object")
					boProps.push(searchProps[i]);
			}
			
			return boProps;
		},
		
		// get all properties, except business object properties
		_getNonBOSearchProps: function(searchProps) {
			var props = [];
			for (i = 0; i < searchProps.length; i++) 
			{
				if (searchProps[i].dataType != "xs:object")
					props.push(searchProps[i]);
			}
