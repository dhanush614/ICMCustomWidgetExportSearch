define(
		[ "dojo/_base/declare", "icm/action/Action", "dojo/dom-style",
				"dijit/form/Button", "dojo/_base/lang", "dojo/_base/array",
				"dojo/parser", "dojox/grid/cells", "dijit/ToolbarSeparator",
				"icm/util/Coordination", "ecm/widget/dialog/BaseDialog",
				"ecm/widget/FilteringSelect", "dojox/grid/_CheckBoxSelector",
				"dojox/grid/DataGrid", "dojox/grid/cells/dijit",
				"dojo/data/ItemFileWriteStore", "dijit/dijit",
				"dijit/form/TextBox", "dojo/date", "dijit/form/DateTextBox",
				"dijit/layout/TabContainer", "dijit/layout/ContentPane",
				"pvr/widget/Layout", "dojo/dom-construct", "dijit/Toolbar",
				"pvr/widget/PropertyTable", "dojo/dom-class",
				"dojo/data/ObjectStore", "dojo/store/Memory",
				"gridx/modules/CellWidget", "gridx/modules/dnd/Row",
				"gridx/modules/Sort", "dojo/aspect", "dojo/dom-attr",
				"dojo/request", "dojo/request/xhr", "dojo/dom", "dojo/on",
				"dojo/mouse",
				"icmcustom/pgwidget/casesearch/dijit/CaseSearchContentPane",
				"icmcustom/action/ICMSearchExportAction",
				"dojo/data/ItemFileReadStore", "icm/util/SearchPayload",
				"icm/base/BasePageWidget", "ecm/model/AttributeDefinition",
				"icm/util/SearchUtil", "dojo/domReady!" ],

		function(declare, Action, domStyle, Button, lang, array, parser, cells,
				ToolbarSeparator, Coordination, BaseDialog, FilteringSelect,
				_CheckBoxSelector, DataGrid, cellsDijit, ItemFileWriteStore,
				dijit, TextBox, date, DateTextBox, TabContainer, ContentPane,
				Layout, domConstruct, Toolbar, PropertyTable, domClass,
				ObjectStore, Memory, CellWidget, Row, Sort, aspect, domAttr,
				request, xhr, dom, on, mouse, CaseSearchContentPane,
				ICMSearchExportAction, ItemFileReadStore, SearchPayload,
				BasePageWidget, AttributeDefinition, SearchUtil) {

			/**
			 * @name icm.pgwidget.casesearch.CaseSearch
			 * @class Provides a widget that is used to render a Case search
			 *        form
			 * @augments dijit._Widget
			 */
			return declare(
					"icmcustom.pgwidget.casesearch.CaseSearch",
					[ BasePageWidget, CaseSearchContentPane ],
					{
						...
						postCreate : function() {
							...
							aspect.after(this, "onClickSearchButton", lang
									.hitch(this, this._onClickSearchButton),
									true);
							aspect.after(this, "executeAdvancedSearch", lang
									.hitch(this, this._onAdvancedButtonClick),
									true);
							this.initialize();
						...
						initialize : function() {
							...
							if (this.solution) {
								this.solution.retrieveCaseTypes(dojo.hitch(
										this, this._retrieveCaseTypesComplete));
							} else {
						...
						_retrieveCaseTypesComplete : function(caseTypes) {
							...
							this.solution.retrieveAttributeDefinitions(dojo
									.hitch(this,
											"_getQuickSearchableProperties"));
						},

						_onClickSearchButton : function() {
							var payload = this.getQuickSearchData();
							payload.caseType = this.widgetProperties.caseTypeName;
							payload.objectStoreName = this.solution
									.getTargetOS().id;
						...
						fireEvent : function(search) {
							var selectQueryString = "SELECT caseProperties,classDescription FROM caseTypeValue WHERE ";
							selectQueryString = selectQueryString.replace(
									"caseTypeValue", search.caseType);
							var searchCondition = "";
								...
								for (var i = 0; i < search.searchCriteria.criterion.length; i++) {
									if (search.searchCriteria.criterion[i].dataType == "xs:timestamp") {
										if (search.searchCriteria.criterion[i].defaultValue[1] != "") {
											var date1 = new Date(
													search.searchCriteria.criterion[i].defaultValue[0]);
											var date2 = new Date(
													search.searchCriteria.criterion[i].defaultValue[1]);
											var date3 = date.add(date2, "day",
													1);
											var beforeDate = this
													.formatDate(date1);
											var afterDate = this
											...
											whereClause = whereClause
													+ ' and '
													+ search.searchCriteria.criterion[i].id
													+ " > "
													+ beforeDate
													+ " and "
													+ search.searchCriteria.criterion[i].id
											...
											var date1 = new Date(
													search.searchCriteria.criterion[i].value);
											var date2 = date.add(date1, "day",
													1);
											var searchDate = this
													.formatDate(date1);
											var afterDate = this
										...
										whereClause = whereClause
												...
												+ search.searchCriteria.criterion[i].id
												+ " LIKE '"
												+ search.searchCriteria.criterion[i].value.shortName
												+ "'"
								...
								if (search.searchCriteria.criterion[0].dataType == "xs:timestamp") {
									...
									if (searchVal == "null"
										...
										var cequery = selectQueryString
												+ search.searchCriteria.criterion[0].id
												+ " LIKE '"
												+ search.searchCriteria.criterion[0].value.shortName
												+ "'"
									}
						...
						_getQuickSearchableProperties : function(propDefs) {
							...
							this.searchViewPropDefs = this
									._constructSearchViewPropertyDefs(propDefs);
							this.allPropDefs = this
									._constructAllSearchablePropertyDefs(propDefs);

							if (this.searchViewPropDefs.length == 0) {
								// If no Search Views exist in the solution, we
								// include Case
								// ID and Date Added in the dropdown.
								dojo
										.forEach(
												icmglobal.wellKnownProperties.quickSearchWellKnowProperties,
												function(property) {
													var attr = new AttributeDefinition(
															{
																...
																cardinality : property.cardinality
																		|| this._searchUtil.CARDINALITY.SINGLE,
							...
							var isSortAllowedOnCaseSearch = icmglobal.configProvider
									.isSortAllowedOnCaseSearch();
							if ((this.searchViewPropDefs.length == 0)
									|| isSortAllowedOnCaseSearch == true) {
								if (quickSearchProperties.length > 1) {
									// the display order of the properties is
									// the
									// alphabetical order
							...
							if (this.widgetProperties
									&& this.widgetProperties.advancedSearchUserSpecifiedSelector == "hideUserSpecified") {
						...
						onExecute : function(model) {
							const
							xlsx = require('xlsx');
							var caseTypeValue = model.CaseType;
							var ceQuery = model.ceQuery;
							var repository = ecm.model.desktop
									.getRepositoryByName(model.ObjectStore);
											...
											for (var j = 0; j < propData.items.length; j++) {
												if (propData.items[j].dataType == "xs:timestamp") {
													myNewItem = {
														...
														dtype : propData.items[j].dataType
																.replace(
																		"xs:timestamp",
																		"datetime")
													};
																					...
																					if (value == store._arrayOfAllItems[a].pname) {
																						...
																						store
																								.setValue(
																										item,
																										'sname',
																										this.item.symbolicName);
																						if (this.item.dataType
																								.includes("xs:timestamp")) {
																							store
																									.setValue(
																											item,
																											'dtype',
										...
										getCaseTypePropData : function() {
											...
											for (var i = 0; i < caseTypes.length; i++) {
												if (caseTypes[i].id == caseTypeValue) {
													caseTypes[i]
															.retrieveAttributeDefinitions(lang
																	.hitch(
																			this,
																			function(
																					retrievedAttributes) {
																				...
																				for (var i = 0; i < rows; i++) {
																					...
																					if (propSymbolicName
																							.includes("_")) {
																						var propList = propSymbolicName
																								.split("_");
																						if (propList[0] == prefix) {
																							propData.items
																									.push(retrievedAttributes[i]);
																						}
																					}
																					if (i == (rows - 1)) {
																						this
														...
														.forEach(
																...
																function(
																	...
																	if (selectedItem != null) {
																		...
																		symNames += ",";
																		displayNames
																				.push(selectedItem.pname);
																	}
																});
											}
											symNames = symNames.replace(
													/,\s*$/, "");
											var symNamesArray = symNames
													.split(',');
											ceQuery = ceQuery.replace(
													"caseProperties", symNames);
																			...
																			function(
																				...
																				if (results.items.length > 0) {
																					...
																					for (var i = 0; i < searchResults.length; i++) {
																						var caseData = JSON
																								.stringify(searchResults[i].attributes);
																						var caseTypesData = JSON
																								.stringify(searchResults[i].attributeTypes);
																						var jsonData = JSON
																								.parse(caseData);
																						var caseTypejson = JSON
																								.parse(caseTypesData);
							...
							initiateTaskDialog1.addButton("Export",
									initiateTaskDialog1.onExport, false, false);
						...
						_constructSearchViewPropertyDefs : function(propDefs) {
							...
							if (this.solution) {
								...
								for (i = 0; i < caseTypes.length; i++) {
									...
									// add SearchView
									fields = type.getSearchView() ? type
											.getSearchView().fields : [];
									for (j = 0; j < fields.length; j++) {
										if (!this._searchUtil.isInArray(
												searchDefs, fields[j].name,
												"id")) {
											var def = this._searchUtil
											...
											for (k = 0; k < icmglobal.wellKnownProperties.searchExceptionProperties.length; k++) {
												if (def.id == icmglobal.wellKnownProperties.searchExceptionProperties[k].name) {
													if (icmglobal.wellKnownProperties.searchExceptionProperties[k].excludedValues
															&& def.choiceList
							...
							/*
							 * if(searchDefs.length > 1) { //the display order
							 * of the properties is the alphabetical order
							 * searchDefs.sort(function(a, b){ return
						...
						_constructAllSearchablePropertyDefs : function(propDefs) {
							...
							for (i = 0; i < propDefs.length; i++) {
								if (propDefs[i].searchable
										&& !propDefs[i].hidden
										&& propDefs[i].dataType != "xs:object"
										&& !this._searchUtil
												.isInArray(
														this._searchUtil.ignoredProperties,
														propDefs[i].id, "id")) {
							...
							if (allDefs.length > 1) {
								// the display order of the properties is the
								// alphabetical
								// order
